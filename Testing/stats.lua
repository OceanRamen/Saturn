...
--[[
TODO: MAKE REGULAR MODE MORE COMPACT. MAKE COMPACT MODE WORK THE SAME WAY AS THE V.1 STAT-TRACKER.
i.e. have the counter in the description of the Joker.
]]
  ...
  if G.STAGE == G.STAGES.RUN and self:has_counter() then
    ...
    if self.ability.name == "Bull" then
      self:set_counter(
        nil,
        self.ability.extra
          * math.max(0, (G.GAME.dollars + (G.GAME.dollar_buffer or 0)))
      )
    end
  ...
  if args.counter_type then
    self.counter = {
      ...
      counter_text = args.counter_text
        or counter_type_table[args.counter_type][1],
      ...
      counter_value_colour = counter_type_table[args.counter_type][2]
      ...
      highscore_text = args.highscore_type
        and (
          args.highscore_text or highscore_type_table[args.highscore_type][1]
        ),
      ...
      highscore_prefix = (
        args.highscore_type and highscore_type_table[args.highscore_type][3]
      ) or "",
      highscore_value = (
        (args.highscore_type and args.highscore_value)
        or (args.highscore_type == "x_mult_scale" and 1)
      ) or 0,
      highscore_old_value = (
        (args.highscore_type and args.highscore_old_value)
        or (args.highscore_type == "x_mult_scale" and 1)
      ) or 0,
      highscore_value_text = args.highscore_type and args.highscore_value_text
        or "None",
      highscore_value_num = args.highscore_type and args.value_num or 1,
      highscore_value_colour = (
        args.highscore_type and highscore_type_table[args.highscore_type][2]
      ) or G.C.UI.TEXT_LIGHT,
      highscore_value_size = args.highscore_type and args.highscore_value_size
        or 0.3,
...
function Card:check_enabled()
  ...
  if self.ability.set == "Joker" and self.area == G.jokers then
    ...
    if
      self:has_counter()
      and Saturn.config.enable_stats
      and Saturn.config.enable_joker_tracking
    then
      if
        (
          self.counter.counter_type == "money gen"
          or self.counter.counter_type == "value gen"
        ) and not checked
      then
        if Saturn.config.enable_tracking_money_generators then
          show_counter = true
        end
        checked = true
      elseif
        (
          self.counter.counter_type == "card gen"
          or self.counter.counter_type == "cards added"
          or self.counter.counter_type == "joker gen"
        ) and not checked
      then
        if Saturn.config.enable_tracking_card_generators then
          show_counter = true
        end
        checked = true
      elseif self.counter.counter_type == "+ chips" and not checked then
        if Saturn.config.enable_tracking_plus_chips then
          show_counter = true
        end
        checked = true
      elseif self.counter.counter_type == "+ mult" and not checked then
        if Saturn.config.enable_tracking_plus_mult then
          show_counter = true
        end
      elseif self.counter.counter_type == "x mult" and not checked then
        if Saturn.config.enable_tracking_x_mult then
          show_counter = true
        end
        checked = true
      elseif
        self.counter.counter_type
        and Saturn.config.enable_tracking_miscellaneous
        and not checked
      then
        show_counter = true
      end
...
-- TODO: FIX NON-COMPACT MODE. REDESIGN COUNTER UI (?)

function Card:generate_counter_definition()
  local definition
  local is_obelisk = (self.ability.name == "Obelisk")
  ...
  local function create_counter_value_node(
    ...
    if counter_type == "x mult" then
      return {
        ...
        nodes = {
          create_text_node("X" .. value, size, G.C.WHITE),
        },
      }
    elseif counter_type == "chips/mult" then
      return {
        n = G.UIT.C,
        config = {
          align = "cm",
        ...
        nodes = {
          create_text_node(
            "+"
              .. (
                self.ability.name == "Walkie Talkie" and value * 10
                or self.ability.name == "Scholar" and value * 20
              ),
            size,
            G.C.BLUE
          ),
          create_text_node("/", size, G.C.L_BLACK),
          create_text_node("+" .. value * 4, size, G.C.RED),
        },
      }
    else
      return {
        ...
        nodes = {
          create_text_node(prefix .. value, size, colour),
        },
      }
    end
  end

  local function create_highscore_node(type, prefix, value, colour, size)
    if type == "x mult scale" then
      return {
        n = G.UIT.C,
        config = {
          align = "cm",
  ...
  local function create_obelisk_node(
    counter_text,
    counter_value_text,
    counter_value_size,
    counter_value_colour,
    obelisk_scale_factor
  )
    local nodes = {
      {
        n = G.UIT.R,
        ...
        nodes = {
          create_text_node(
            counter_text .. ":",
            counter_value_size
              * (1 - ((1 - obelisk_scale_factor) * (#counter_value_text - 1))),
            counter_value_colour
          ),
        },
      },
  ...
  if not Saturn.config.enable_compact_stats_display then
    if self:is_scaling() and self.counter.locked then
      definition = create_base_definition()
      local highscore_node = {
        n = G.UIT.R,
        config = {
          align = "cm",
          minh = 0.5, -- Ensure the minimum height is sufficient
        },
        nodes = {
          create_text_node(
            self.counter.highscore_text,
            self.counter.highscore_text_size,
            self.counter.highscore_text_colour
          ),
        },
      }
        ...
        nodes = {
          {
            ...
            nodes = create_highscore_node(
              self.counter.highscore_type,
              self.counter.highscore_prefix,
              self.counter.highscore_value,
              self.counter.highscore_value_colour,
              self.counter.highscore_value_size
            ),
          },
        },
      }
      ...
      table.insert(definition.nodes[1].nodes[1].nodes[1].nodes, highscore_node)
      ...
      local counter_node = {
        ...
        nodes = {
          create_text_node(
            self.counter.counter_value_num > 1
                and self.counter.counter_text .. "s"
        ...
        nodes = {
          {
            ...
            nodes = create_counter_value_node(
              self.counter.counter_type,
    ...
    definition = create_base_definition()
    if self:is_scaling() and self.counter.locked then
      local highscore_node = create_text_node(
        self.counter.highscore_text .. ":",
        self.counter.highscore_text_size,
        self.counter.highscore_text_colour
      )

      local value_node = create_highscore_node(
        self.counter.highscore_type,
        self.counter.highscore_prefix,
        self.counter.highscore_value,
        self.counter.highscore_value_colour,
  ...
  if not self.counter.locked then
    ...
    if self:is_scaling() then
      config = {
        ...
        offset = {
          x = 0,
          y = offset_below and self.counter.highscore_offset
            or self.counter.highscore_offset * -1,
        },
        type = "bm",
      }
    else
...
function Card:set_counter(counter, highscore)
  if self then
    ...
    if self:has_counter() then
      ...
      if not highscore then
        highscore = self.counter.highscore_value
      end
...
function Card:counter_bonus()
  ...
  if self.ability.set == "Joker" then
    ...
    if
      self.ability.name == "Cloud 9"
      and self.ability.nine_tally
      and self.ability.nine_tally > 0
    then
    ...
    if
      self.ability.name == "Delayed Gratification"
      and G.GAME.current_round.discards_used == 0
      and G.GAME.current_round.discards_left > 0
    then
      self:increment_counter(
        G.GAME.current_round.discards_left * self.ability.extra
      )
    end
  end
end
...
function Card:calculate_counters(context)
  ...
  elseif context.setting_blind and not self.getting_sliced then
    ...
    if
      self.ability.name == "Burglar"
      and not (context.blueprint_card or self).getting_sliced
    then
      self:increment_counter(self.ability.extra)
    end
    if
      self.ability.name == "Marble Joker"
      and not (context.blueprint_card or self).getting_sliced
    then
      self:increment_counter(1)
    end
  ...
  elseif context.using_consumeable then
    ...
    if
      self.ability.name == "Fortune Teller"
      and not context.blueprint
    ...
    if
      self.ability.name == "Constellation"
      and not context.blueprint
  ...
  elseif context.discard then
    ...
    if
      self.ability.name == "Trading Card"
      and not context.blueprint
      and G.GAME.current_round.discards_used <= 0
    ...
    if
      self.ability.name == "Hit the Road"
      and not context.other_card.debuff
    ...
    if
      self.ability.name == "Mail-In Rebate"
      and not context.other_card.debuff
    ...
    if
      self.ability.name == "Faceless Joker"
      and context.other_card == context.full_hand[#context.full_hand]
    then
      local face_cards = 0
  ...
  elseif context.individual then
    if context.cardarea == G.play then
      ...
      if
        self.ability.name == "Scary Face"
        and (context.other_card:is_face())
      then
        self:increment_counter(self.ability.extra)
      end
      if
        self.ability.name == "Smiley Face"
        and (context.other_card:is_face())
      then
        self:increment_counter(self.ability.extra)
      end
      if
        self.ability.name == "Golden Ticket"
        and context.other_card.ability.name == "Gold Card"
      then
        self:increment_counter(self.ability.extra)
      end
      if
        self.ability.name == "Scholar"
        and context.other_card:get_id() == 14
      then
        self:increment_counter(1)
      end
      if
        self.ability.name == "Walkie Talkie"
        and (
    ...
    if context.cardarea == G.hand then
      if
        self.ability.name == "Shoot the Moon"
  ...
  elseif context.repetition then
    if context.cardarea == G.play then
      if
        self.ability.name == "Sock and Buskin"
        and (context.other_card:is_face())
      then
        self:increment_counter(self.ability.extra)
      end
      if
        self.ability.name == "Hanging Chad"
        and (context.other_card == context.scoring_hand[1])
      then
        self:increment_counter(self.ability.extra)
      end
      if
        self.ability.name == "Dusk" and G.GAME.current_round.hands_left == 0
      then
        self:increment_counter(self.ability.extra)
      end
  ...
  elseif context.other_joker then
    if
      self.ability.name == "Baseball Card"
      and context.other_joker.config.center.rarity == 2
      and self ~= context.other_joker
    then
      self:increment_counter(self.ability.extra)
    end
  else
    if context.cardarea == G.jokers then
      if context.before then
        ...
        if
          self.ability.name == "To Do List"
          and context.scoring_name == self.ability.to_do_poker_hand
        then
          self:increment_counter(self.ability.extra.dollars)
        end
        if
          self.ability.name == "DNA"
        ...
        if self.ability.name == "Loyalty Card" then
          local loyalty_remaining = (
            self.ability.extra.every
            - 1
            - (G.GAME.hands_played - self.ability.hands_played_at_create)
        ...
        if
          self.ability.name ~= "Seeing Double"
          and self.ability.name ~= "Obelisk" -- Lucky Cat maybe
          and self.ability.x_mult > 1
          and (
            self.ability.type == ""
            or next(context.poker_hands[self.ability.type])
          )
        then
        ...
        if
          ...
          and next(context.poker_hands[self.ability.type])
        then
        ...
        if
          self.ability.name == "Half Joker"
          and #context.full_hand <= self.ability.extra.size
        then
        ...
        if
          self.ability.name == "Acrobat"
          and G.GAME.current_round.hands_left == 0
        then
          self:increment_counter(self.ability.extra)
        end
        if
          self.ability.name == "Mystic Summit"
          and G.GAME.current_round.discards_left
            == self.ability.extra.d_remaining
        then
          self:increment_counter(self.ability.extra.mult)
        end
        if
          self.ability.name == "Banner"
          and G.GAME.current_round.discards_left > 0
        then
          self:increment_counter(
            G.GAME.current_round.discards_left * self.ability.extra
        ...
        if self.ability.name == "Flower Pot" then
          ...
          for i = 1, #context.scoring_hand do
            if context.scoring_hand[i].ability.name == "Wild Card" then
              if
                context.scoring_hand[i]:is_suit("Hearts")
                and suits["Hearts"] == 0
              then
                suits["Hearts"] = suits["Hearts"] + 1
              elseif
              ...
              then
                suits["Spades"] = suits["Spades"] + 1
              elseif
                context.scoring_hand[i]:is_suit("Clubs")
                and suits["Clubs"] == 0
              then
                suits["Clubs"] = suits["Clubs"] + 1
              end
        ...
        if
          self.ability.name == "Erosion"
          and (G.GAME.starting_deck_size - #G.playing_cards) > 0
        then
          self:increment_counter(
            self.ability.extra * (G.GAME.starting_deck_size - #G.playing_cards)
          )
        end
        ...
        if
          self.ability.name == "Stone Joker" and self.ability.stone_tally > 0
        then
        ...
        if
          self.ability.name == "Steel Joker" and self.ability.steel_tally > 0
        then
          self:increment_counter(
            1 + self.ability.extra * self.ability.steel_tally
          )
          self:set_counter(
            nil,
            1 + self.ability.extra * self.ability.steel_tally
        ...
        if self.ability.name == "Blackboard" then
          ...
          for k, v in ipairs(G.hand.cards) do
            ...
            if
              v:is_suit("Clubs", nil, true) or v:is_suit("Spades", nil, true)
            then
        ...
        if
          self.ability.name == "Fortune Teller"
          and G.GAME.consumeable_usage_total
          and G.GAME.consumeable_usage_total.tarot > 0
        then
